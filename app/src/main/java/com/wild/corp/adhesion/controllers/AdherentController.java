...
//    }
    ...
    public ResponseEntity<?> update(Authentication principal, @RequestBody AdherentLite adherent) {
        ...
        if(adherent.getId() == null){
            log.info("newAdherentDansTribu by " + principal.getName() + " for adherent " + adherent.getPrenom());

            Adherent newAdherent = adherentServices.saveNewAdherent(adherent);
            adherentServices.addModification(principal.getName(), newAdherent.getId(), "Ajout du nouvel adhérent");
